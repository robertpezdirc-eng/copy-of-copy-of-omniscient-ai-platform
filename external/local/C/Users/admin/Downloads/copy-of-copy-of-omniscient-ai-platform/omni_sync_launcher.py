        ...
        while self.running:
            try:
                ...
                if (current_devices != last_device_count or
                    ...
                    print("
 System Status:"                    print(f"   Scan Cycles: {current_scan_cycles}")
                    ...
                    if current_devices > 0:
                        print("   Recent Devices:"                        devices = get_devices()
...
def demo_device_discovery(duration=60):
    ...
    if 'error' not in status:
        print("
 Final Results:"        print(f"   Total Scan Cycles: {status['sync_core'].get('scan_cycles', 0)}")
        ...
        if status['devices']['recent']:
            print("   Recently Discovered Devices:"            for device in status['devices']['recent']:
