...
class RedisRateLimiter(BaseHTTPMiddleware):
    ...
    def __init__(self, app, redis_client: Optional[redis.Redis] = None, redis_getter: Optional[Callable[[], Optional[redis.Redis]]] = None):
        ...
        # Enable if explicitly set, otherwise auto-enable when Redis URL provided
        self.enabled = (
            settings.rate_limiting_enabled
            if settings.rate_limiting_enabled is not None
            else settings.redis_url is not None
        )
        # Configurable window
        self.window = max(1, int(settings.rate_limit_window_seconds))

        # Tier limits (requests per minute)
        self.tier_limits = {
            "free": int(settings.rate_limit_free_per_min),
            "basic": int(settings.rate_limit_basic_per_min),
            "premium": int(settings.rate_limit_premium_per_min),
            # unlimited when configured; otherwise use a very high ceiling
            "master": None if settings.rate_limit_master_unlimited else int(10**9),
        }
